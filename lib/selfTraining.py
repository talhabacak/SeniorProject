...
class SelfTraining2():
    def __init__(self,model,train_x,train_y,valid_x,valid_y,test_x,test_y,unlabel_x,unlabel_y,class_number=2, perEpoch_label=200,path_output="model_best.h5",activation="sigmoid"):
        """
        ...
        self.X_val = np.array(valid_x)
        self.X_test = np.array(test_x)
        self.y_train = np.array(train_y)
        self.y_val = np.array(valid_y)
        self.y_test = np.array(test_y)
        self.X_unlabel = np.array(unlabel_x)
        self.y_unlabel = np.array(unlabel_y)
        self.lenUnlabel_y = len(self.y_unlabel)

        self.class_number = class_number
        self.activation = activation
        self.perEpoch_label = perEpoch_label
        self.path_output = path_output
        self.history = []
        self.f1Score = []
        self.loss = []
    ...
    def run(self):
        ...
        maxF1 = 0
        self.startTime = time.time()
        while changeF1 < 5 or len(self.X_unlabel) > 0:
            try:
                self.history.append(self.model.fit(self.X_train, self.y_train, epochs=1, 
                          validation_data=(self.X_val, self.y_val)))
            except Exception as e:
